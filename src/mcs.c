...
#define MAX_JERRY_STRING_SIZE 2048
...
void mcs_tcp_init(void (*mcs_tcp_callback)(char *))
{
    ...
    for (;;) {
        ...
        if (0 == strncmp (arr[3], "Javascript", 10)) {
            fota_mode = 1;
            // char data_buf = malloc(10048);
            char *data_buf[2048] = {0};
            strcat(data_buf, arr[0]);
            strcat(data_buf, ",");
            strcat(data_buf, arr[1]);
            strcat(data_buf, ",");
            strcat(data_buf, arr[2]);
            strcat(data_buf, ",");
            strcat(data_buf, arr[3]);
            strcat(data_buf, ",");
            printf("data_buf: %s\n", data_buf);
            char *s = mcs_replace(rcv_buf, data_buf, "");
            jerry_cleanup();
            printf("s: %s\n", s);
            ...
            jerry_api_eval (s, strlen (s), false, false, &eval_ret);
